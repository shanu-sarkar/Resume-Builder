...
//This mapStateToProps func transfer the Redux state to the component properties. It obtains personal info from the Redux state.
const mapStateToProps = (state) => ({
  personalInfo: state.personalInfoReducer.personalInfo,
});

//mapDispatchToProps functions map dispatch actions to component props. It includes the onAddPersonalInfo method, which initiates an action to add personal info to the Redux store.
const mapDispatchToProps = (dispatch) => ({
  onAddPersonalInfo: (details) => dispatch(addPersonalInfo(details)),
});

//BootstrapDialog is a stylized dialogue component based on Material-ui dialogue component. It includes an avatar for a profile img and the button to modify the image, which opens the dialogue box.
...
const PersonalInfoComponent = (props) => {
  const [loading, setLoading] = useState(false); // The component uses the useState hook to manage local state for loading, imgSnackbar, img, setSotreImage, and open.
  ...
  //handleNext handles form submission.
  ...
  //BootstrapDialogTitle is styled dialogue component based on Material-ui Dialogue component.
  ...
  //This func is triggered when the user saves the cropped img. It updates the setStoreImage state with the new picture.
  const saveImage = () => {
    setSotreImage([{ img }]);
    setOpen(false);
  };
